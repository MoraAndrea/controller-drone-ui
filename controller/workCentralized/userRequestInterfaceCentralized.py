...
from controller.UserGuiResources.logger import App, ConsoleUiLogger
...
from controller.workCentralized.runKubernetesCentralized import runKube
...
class Application(Frame):
    ...
    def run(self):
        self.text_file.delete('1.0', END)
        pods_info=runKube(self, self.file, self.value_app, self.value_param, self.PathVideo[self.value_param][0])
        ...
        if self.podsRun is None:
            ...
            if self.file is not None:
                ConsoleUiLogger.submit_message('INFO', "Send request for application describe in file: " + self.file.name)
            else:
                if self.value_app and self.value_param is not None:
                    ConsoleUiLogger.submit_message('INFO', "Request for " + self.value_app + " " + self.value_param + " send...")
            self.btn_close.config(state=NORMAL)

    # delete all pod created
    def close(self):
        self.text_file.delete('1.0', END)
